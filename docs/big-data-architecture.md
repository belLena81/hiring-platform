...
Account deletion writes a durable erasure request in the same receipt-owned Mongo transaction as the account tombstone and recruiter job-close events. The runnable batch currently uses an empty marker source; the durable marker reader, fail-closed behavior, purge, Gold rebuild, and snapshot republish are not implemented yet. These are required before erasure is effective in analytics.
