...
The schema includes Hiring GraphQL operations and typed payloads for jobs, applications, status transitions, cursor connections, application history, and account lifecycle. `User.profile` is the nullable `UserProfile` union of `CandidateProfile` and `RecruiterProfile`; clients select role-specific fields with inline fragments. `bootstrapAdmin` creates the first singleton Admin; `signUp` then accepts only Candidates and Recruiters, while Admin signup returns `ADMIN_BOOTSTRAP_ONLY`. `login` issues short-lived HS256 bearer tokens when `AUTH_JWT_HS256_SECRET` is configured. `me`, `updateMyProfile`, and `deleteMyAccount` derive identity from the token subject. The token `sub` must match a currently Active stored user, and authorization derives the actor role from that stored user rather than from role claims. Missing, invalid, expired, wrong issuer/audience, deleted-user, or unknown-user tokens return sanitized unauthorized payloads. Passwords are stored only as Argon2id hashes; email is optional and is not a login identity.
