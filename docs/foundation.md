...
For login and signup rate limiting, the default empty `http.trusted-proxy-cidrs` list uses the TCP peer address and ignores `Forwarded`. When the application is reachable only through known reverse proxies, configure their immediate peer CIDRs, for example `http.trusted-proxy-cidrs = ["10.0.0.0/8"]`. A configured proxy must strip or replace client-supplied `Forwarded` before appending its observed connection peer. The application uses concrete RFC 7239 `Forwarded: for=` hops only after the TCP peer matches the allowlist; it scans from right to left, skips malformed, unknown, obfuscated, or incomplete hops, and uses the nearest parseable non-proxy address. If no such hop exists, it falls back to the peer address. Allowlist only networks that can directly connect to the application, restrict direct access at the network layer, and never use `/0`. The limiter remains process-local, so replicas do not share buckets.
...
Each request owns its admission permit and GraphQL context resource; the dispatcher is server-lifetime infrastructure. Neither request release nor the HTTP deadline can cancel Sangria Futures that have already started, so the shorter resolver-service bound is the application-level mitigation. The installed Ember transport awaits response computation before its next socket operation: a peer disconnect alone is not an immediate cancellation signal. Disconnected probes are bounded by the two-second probe deadline; stalled request bodies by the five-second request deadline. Real socket-reset tests check eventual cleanup and subsequent successful requests.
