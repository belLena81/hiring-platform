...
- User outcome: the domain service operational hiring workflow is available through a production-style GraphQL contract with bounded pagination, direct query values, operation-specific mutation unions, service-owned authorization, request-scoped batching, and documented MongoDB query/index evidence.
...
- Target requirements: `docs/development-milestones.md` Hiring GraphQL API requires queries, mutations, inputs, mutation result unions, real GraphQL errors, RBAC, cursor pagination, nested resolvers, request-scoped batching, UC01 structured job search, GraphQL exposure for domain service use cases, documented access patterns, and `explain("executionStats")` evidence.
...
- Cursor decision: API cursors are opaque authenticated v3 base64url keysets. Jobs and applications encode the `j`/`a` tag, `createdAt`, and UUID; application history encodes the `e` tag, `occurredAt`, and UUID; user listings use the `u` tag and `createdAt`. A truncated HMAC-SHA256 authenticates the payload. Invalid, malformed, cross-type, legacy, or unauthorized cursors return typed sanitized errors and must not bypass ownership checks.
...
- Schema additions:
  ...
  - Types: `User`, `CandidateProfile`, `Job`, `Location`, `Application`, `ApplicationEvent`, status enums, role enum, `PageInfo`, connection/edge types, typed inputs, direct query values, and operation-specific mutation result unions.
- Authorization:
  ...
  - Application services remain the authorization boundary. Resolvers parse IDs/inputs, validate cursors/page size, call services, and translate expected domain outcomes to unions while mapping other failures to sanitized GraphQL errors.
...
- Pagination and validation:
  ...
  - Expected validation and domain outcomes return union members without raw driver messages; authentication, account, availability, repository, and search failures use `errors[].extensions.code`.
...
|---|---|---|---|---|
| HGQL-AC01 | Given the Hiring GraphQL API schema is added, when SDL is exported, then Foundation fields remain compatible and hiring operations appear with typed inputs, operation-specific result unions, enums, connections, and sanitized errors | `HiringGraphQLSchema`; `hiring.graphql`; `HiringGraphQLContractSpec` | `sbt test` | PASS: SDL fixture updated with mutation input objects, unions, and direct query/connection fields |
| HGQL-AC02 | Given missing or invalid authentication, when a hiring operation is executed over HTTP, then it returns a sanitized top-level GraphQL error with `extensions.code` and no domain write occurs | `JwtActorAuthenticatorSpec`; `HiringApiRoutesSpec`; `HiringGraphQLAccessSpec` | `sbt test` | PASS: missing, malformed, tampered, expired, wrong issuer/audience, future `nbf`, and unknown-user tokens fail closed; role claims are ignored in favor of stored user role |
...
| HGQL-AC06 | Given duplicate, closed-job, invalid-transition, missing-feedback, or missing-reason outcomes, when mutations are executed, then expected domain outcomes are union members while non-domain failures expose stable top-level error codes and no raw driver/internal details | `HiringGraphQLSchema`; `HiringGraphQLAccessSpec`; existing service/repository negative tests | `sbt test`; `sbt 'IntegrationTest / test'` | PASS: GraphQL mutation tests cover union outcomes and top-level error extensions; repository tests cover no partial writes |
...
- API contract: add schema types, inputs, mutation result unions, SDL snapshot, and executable fixtures while preserving `health` and `readiness`.
