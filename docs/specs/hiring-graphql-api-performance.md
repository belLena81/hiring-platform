...
Status: in review / partially blocked - GraphQL contract, resolver tests, repository query contracts, MongoDB setup/index updates, and local unit/integration evidence are implemented. Successful served HTTP hiring workflows remain blocked until a separately scoped authentication source supplies trusted `ActorContext`. The user explicitly selected no temporary trusted-header bridge and no JWT/login implementation in this capability spec.
...
- Dependencies: domain services and MongoDB repositories are the baseline. Domain-service final Code Reviewer, Security Engineer, and QA verdicts are still pending in `docs/specs/phase-2-domain-mongodb.md` and must be reconciled before Hiring GraphQL API can be marked done.

## Source context and decisions

- Verified implementation: Foundation serves only `health` and `readiness` in `HiringGraphQLSchema`, with deterministic SDL and operation fixtures. Domain-service source contains `ActorContext`, `JobService`, `ApplicationService`, typed `UseCaseError`, MongoDB repositories, transactions, bounded application pagination, and named list indexes.
...
| HGQL-AC02 | Given no authentication source exists, when an authenticated hiring operation is executed over HTTP, then it returns a sanitized unauthorized GraphQL payload and no domain write occurs | `RequestContext`; `HiringGraphQLAccessSpec` | `sbt test` | PASS: mutation without `ActorContext` returns `UNAUTHORIZED`; successful served HTTP hiring remains blocked by no auth bridge |
| HGQL-AC03 | Given injected Candidate, Recruiter, and Admin `ActorContext` values in resolver tests, when domain service use cases are executed, then service-owned authorization and ownership rules are enforced without client-controlled owner IDs | `HiringGraphQLAccessSpec`; existing service specs | `sbt test` | PARTIAL: injected Candidate search/list paths plus existing service authorization pass; broader Admin/recruiter GraphQL matrix still needs independent review/follow-up coverage |
...
- Current blockers: successful served hiring GraphQL operations require a future auth slice to provide trusted `ActorContext`; domain service final independent reviews are still pending; real performance SLO claims require a workload benchmark not included here.
