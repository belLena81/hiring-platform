...
- Mongo transaction retries now delegate delay and attempt bookkeeping to cats-retry while retaining Mongo-specific operation/commit label classification and abort semantics: `src/main/scala/com/example/graphQL/cats/repository/mongo/MongoHiringRepositories.scala`.
...
- Completed: `MongoTransactionRunner` uses cats-retry v4 capped exponential policies for transaction and commit attempts; focused classification and bounded-attempt evidence passes.
