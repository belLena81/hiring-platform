...
- Normal job/profile writes enqueue embedding regeneration and do not call Voyage synchronously. Missing or stale embeddings are typed search errors, not silent fallbacks that widen visibility.
...
| VHS-AC05 | Given job/profile content changes, when the write succeeds, then embedding regeneration is queued asynchronously and mutation latency is independent from Voyage latency | `JobServiceSpec`; `EmbeddingPipelineSpec` | `sbt test` | PARTIAL/PASS: job create/update enqueue after successful writes and pipeline processes bounded queue; no candidate-profile mutation entrypoint exists yet |
| VHS-AC06 | Given canonical searchable content has not changed, when regeneration runs, then `sourceHash` prevents an unnecessary Voyage call | `EmbeddingPipelineSpec`; `SemanticSearchServiceSpec`; `MongoHiringCodecsSpec` | `sbt test` | PASS for hash skip, same-version stale-hash rejection, and embedding metadata codec compatibility |
| VHS-AC07 | Given Atlas vector search runs, when `$vectorSearch` is built, then it uses configured index names, 1024 dimensions, metadata filters, bounded `numCandidates`, and `limit <= numCandidates` | `MongoSemanticSearchRepository`; config tests; Mongo setup integration | source inspection; `sbt test`; `sbt 'IntegrationTest / test'` | PARTIAL: model/version filters cover search and recommendations, 1024 is enforced, metadata indexes/migration are asserted; live Atlas `$vectorSearch` execution not run |
...
- Allowed write paths: `src/main/scala/com/example/graphQL/cats/domain`, `application`, `api/graphql`, `config`, `infrastructure/mongo`, `infrastructure/embedding`, tests, docs/specs, `docs/api.md`, `docs/mongodb-design.md`, and relevant fixtures.
...
- Completed criteria and changed files: added additive GraphQL vector/hybrid search contract, search/embedding domain metadata, semantic search service, bounded embedding pipeline, Voyage adapter/config, Mongo embedding codec/update/vector query paths, metadata indexes, SDL fixture, and focused tests.
- Latest commands/results and their scope: after Code Reviewer/Security/QA fixes, committed config sanitization, and stale-hash validation, `sbt test` passed 160/160 unit tests; `sbt 'IntegrationTest / test'` passed 31/31 integration tests with the known handled bind-conflict trace in `HiringPlatformServerSpec`; `python3 scripts/check-skills.py` validated 8 project-local skills; `git diff --check` passed.
...
- Code Reviewer verdict and scope: PASS after sourceHash staleness, enqueue, metadata filter, dimension, vector filter, and migration/index evidence fixes; live Voyage/Atlas remains a documented gap.
- Security Engineer verdict and scope: PASS after actor resolution, PII minimization, input budget, config sanitization, and safe defaults fixes; live credentials/provider behavior and served auth remain runtime gaps.
- Final independent QA verdict: BLOCKED on VHS-AC09 only because live Voyage API + Atlas `$vectorSearch` verification was not run; local behavior and regression gates passed.
