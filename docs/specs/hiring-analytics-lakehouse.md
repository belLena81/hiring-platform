...
| HAL-02 | Lifecycle, duplicate-hire, malformed, or conflicting events arrive | Canonical lifecycle facts aggregate; invalid/conflicting input quarantines and prevents publish | Transformation tests pass for malformed/conflicting input and duplicate-hire exclusion; Gold-to-Mongo publish wiring is not implemented |
...
| HAL-06 | Retention passes | Bronze/quarantine expire at seven days; Silver/Gold/snapshots and HMAC deletion markers expire at 30/31 days | Batch expiry is implemented for Bronze, quarantine, and Silver. Gold/snapshot/marker retention enforcement remains unimplemented |
| HAL-07 | A local deterministic workload executes | Dataset counts, physical plan, shuffle/file counts, duration, memory, storage, and reconciliation are reported | Opt-in local Compose batch deployment exists; workload harness and measurements are pending |

## Checkpoint and review

- Current checks: root `Test/compile`; focused root service tests (18 passed); isolated Spark transformation tests (6 passed) under Java 17; `git diff --check`. Compose configuration verification remains required after runtime edits.
