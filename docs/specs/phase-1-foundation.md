...
`IOApp` owns Mongo client and Ember via `Resource`. Invalid configuration, client construction, or HTTP bind failure emits a sanitized diagnostic, unwinds acquisition, and exits nonzero. Unreachable MongoDB leaves HTTP running. Ember drains in-flight responses for ten seconds, then cancels remaining requests; finalization follows rather than being skipped to meet a hard ten-second deadline.
...
HTTP: POST `/graphql` accepts JSON requests and negotiates `application/graphql-response+json` or `application/json` responses (request charset permitted). Accept absent prefers `application/graphql-response+json`; otherwise the highest-quality supported media range is selected, with the new GraphQL response media preferred on ties. Quality defaults to 1 and must be 0..1 with at most three decimal places; malformed/duplicate quality parameters or excluded supported media return 406. Required query is a string; missing/null variables means an empty object; missing/null operationName means absent. Invalid shapes, batches, syntax, schema/operation/variable validation, and query budgets return 400. GET `/graphql` returns 405; unsupported request media 415, oversized bodies 413, overload 503, deadline 504, unexpected server failure 500. Normal field execution errors use sanitized GraphQL errors with HTTP 200. No raw exception details are exposed.

GET `/health` returns 200 and `{ "status": "UP" }`; GET `/ready` returns 200/503 with READY/NOT_READY. Both HTTP probes bypass the GraphQL concurrency gate. GraphQL health still uses ordinary GraphQL admission. GET `/schema.graphql` exports the same deterministic SDL checked by contract tests without database I/O. GraphQL introspection and representative operations support external API tools. No UI routes/assets/Node build are included; `/graphiql` returns 404. No wildcard CORS.

Configuration uses PureConfig's default source and Typesafe Config's standard `application.conf`, `config.file`, and `config.resource` loading and precedence. HOCON source composition and `${?VAR}` environment substitutions are resolved at application startup; `AppConfig` retains only application validation and safe error classification. Ignored `local.conf` provides local run values such as HTTP bind address and local MongoDB URI when selected through the standard config mechanism. Packaged `application.conf` may require environment-backed cloud/runtime values and sensitive credentials instead of carrying fake local placeholders; it may also provide literal defaults for non-sensitive startup knobs such as log level, disabled feature flags, provider model, and bounded runtime settings. Missing required packaged/env/local values fail startup with sanitized config errors rather than being hidden by placeholder local defaults. Disabling masking requires a loopback bind address. Validate numeric IP, port 1..65535, database name, URI, TRACE/DEBUG/INFO/WARN/ERROR log level and strict logging-policy flags. `.env` files are not auto-loaded; supply safe examples and explicit export instructions.

Fixed local safety budgets: 16 admitted GraphQL requests, fail-fast excess rejection; 64 KiB streamed bodies; five-second total request deadline including body consumption; selected-operation depth 16 and complexity 1000 enforced by Sangria execution reducers. Syntax/schema validation covers the whole document through Sangria's parser and validator. Standard introspection is a contract fixture. http4s middleware owns admission and deadline cleanup; Ember owns graceful draining.
...
| P1-AC05 | Bounded malformed/large/slow/overload/deadline requests and sanitized errors | HiringApiRoutesSpec, RequestContextSpec | PASS: request-size, media, quality precedence, 500/field errors, middleware saturation, probe bypass, and deadline coverage |
| P1-AC06 | Real MongoDB probes; cancellation registration races and actual Ember disconnect cleanup | PublisherBridgeSpec, RequestContextSpec, HiringApiRoutesSpec, HiringPlatformServerSpec, MongoDatabaseProbeIntegrationSpec | PASS: registration races, rejection during blocked teardown, Ember-owned graceful draining, and deadline-driven real socket RST cleanup |
...
Completion checkpoint (2026-09-16): working tree was clean at `6f07e55` before these fixes. The existing Compose MongoDB container was stopped and ports 8080/27017 were unused before smoke testing; preserve its bind-mounted data and restore its stopped state afterward. Request scope/media negotiation and runtime logging/process tests have disjoint specialist owners. Earlier 40-unit/10-integration passes are historical evidence, not final verification of this completion patch.
