...
Configuration loads `application.conf` and overlays ignored `local.conf` when present before resolving `${VAR}` environment placeholders. Local values can live in ignored `local.conf`; deployed environments can provide host, port, MongoDB URI, JWT secret, and provider settings through process environment variables. Disabling masking requires a loopback bind address; request payload capture additionally requires masking disabled. Validate numeric IP, port 1..65535, database name, URI, INFO/WARN/ERROR log level and strict logging-policy flags. `.env` files are not auto-loaded; supply safe examples and explicit export instructions.
