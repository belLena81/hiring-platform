...
| ETR-02 | Given an HTTP GraphQL request, when it crosses resolver, service, repository, and Mongo boundaries, then spans share the middleware-owned OpenTelemetry trace ID with parent relationships. | `ServerMiddleware`, `HiringApiRoutes`, `HiringGraphQLSchema`, `RequestContext`, and `TracedHiringServices`; `TracePropagationSpec` | Focused HTTP-to-job-service parent test | Partial: account and Mongo boundary coverage remain pending |
