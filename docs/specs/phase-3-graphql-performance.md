...
Status: draft / blocked - full Phase 3 scope is specified, but served hiring GraphQL operations remain blocked until a separately scoped authentication source supplies trusted `ActorContext`. The user explicitly selected no temporary trusted-header bridge and no JWT/login implementation in this phase spec.
...
|---|---|---|---|---|
| P3-AC01 | Given the Phase 3 schema is added, when SDL is exported, then Foundation fields remain compatible and hiring operations appear with typed inputs, payloads, enums, connections, and sanitized errors | Not implemented | SDL snapshot plus operation fixture tests | Not run |
| P3-AC02 | Given no authentication source exists, when an authenticated hiring operation is executed over HTTP, then it returns a sanitized unauthorized GraphQL payload and no domain write occurs | Not implemented | GraphQL route/resolver tests with absent `ActorContext` | Blocked by no auth bridge decision for successful HTTP hiring paths |
| P3-AC03 | Given injected Candidate, Recruiter, and Admin `ActorContext` values in resolver tests, when Phase 2 use cases are executed, then service-owned authorization and ownership rules are enforced without client-controlled owner IDs | Not implemented | Resolver/service integration tests | Not run |
| P3-AC04 | Given a Candidate searches jobs with structured filters, when results are returned, then only Open jobs are visible, ordering is deterministic, page size is bounded, and cursors are opaque | Not implemented | GraphQL tests plus MongoDB explain evidence | Not run |
| P3-AC05 | Given a Candidate submits an application, when the job is Open and no duplicate exists, then the payload returns a Created application and Phase 2 transaction invariants persist application plus initial history | Not implemented | GraphQL test plus existing service/repository integration coverage | Not run |
| P3-AC06 | Given duplicate, closed-job, invalid-transition, missing-feedback, or missing-reason errors, when mutations are executed, then payloads expose stable typed error codes/messages and no raw driver/internal details | Not implemented | Mutation negative tests and log/error inspection | Not run |
| P3-AC07 | Given Candidate and Recruiter list operations, when optional status filters and cursors are used, then pagination is keyset-based, `hasNextPage` is derived with `first + 1`, and invalid cursors are rejected safely | Not implemented | GraphQL and repository integration tests | Not run |
| P3-AC08 | Given applications are queried with nested candidate/job/recruiter fields, when a page contains multiple edges, then request-scoped batching prevents N+1 queries and keeps caches request-local | Not implemented | Instrumented fake repository tests plus real MongoDB query-count evidence where practical | Not run |
| P3-AC09 | Given every major GraphQL read operation, when MongoDB integration tests run, then query shape, winning index, execution stats, and fixture limits are recorded in the spec checkpoint | Not implemented | `IntegrationTest / test` explain assertions | Not run |
| P3-AC10 | Given Phase 3 implementation is complete, when local gates run, then skill validation, unit tests, integration tests, schema/fixture checks, diff check, Code Reviewer, Security Engineer, and final QA pass | Not implemented | `python3 scripts/check-skills.py`; `sbt test`; `sbt 'IntegrationTest / test'`; `git diff --check`; reviews | Not run |
...
- Current status: draft / blocked. The specification is prepared for full Phase 3 scope, with successful HTTP hiring execution blocked by the no-auth-bridge decision.
- Completed criteria and changed files: none; this document is planning/specification only.
- Latest commands/results and their scope: not run for implementation. Before coding, reconcile current source and Phase 2 review status.
- Next concrete action: run readiness reviews by Architect, Data Engineer, and Security Engineer, then either authorize an authentication slice or proceed with schema/resolver tests that inject `ActorContext` below the HTTP boundary.
