...
- Required document fields:
  - `users`: `_id`, `schemaVersion`, `emailCanonical`, `emailDisplay`, `name`, `role`, `profile?`, `adminSingletonKey?`, `createdAt`, `updatedAt`, `disabledAt?`.
...
| P2-AC01 | Given job create/update/publish/close behavior, when pure transitions run, then `cats.data.State` produces the expected job state and rejected transitions leave state unchanged | `JobLifecycleSpec`; `JobServiceSpec` | `sbt test` | PASS: 112 unit tests passed; focused red was captured before adding `JobLifecycle` |
...
| P2-AC04 | Given repository setup, when migrations run twice, then `schema_migrations` records the setup and all named unique/list/history indexes exist idempotently with expected keys and partial filters | `MongoHiringRepositoriesIntegrationSpec` | `sbt 'IntegrationTest / test'` | PASS: setup reruns, migration record, and named indexes are asserted |
| P2-AC05 | Given concurrent duplicate application submissions for the same candidate/job, when both finish, then exactly one application and initial event exist and duplicates return sanitized typed errors | Unique index plus repository duplicate path; synchronized race test still pending | `sbt test`; `sbt 'IntegrationTest / test'` | PARTIAL: duplicate key maps to `DuplicateApplication`; no synchronized concurrent replica-set race test yet |
| P2-AC06 | Given an open job racing with closure, when closure commits before the authoritative submit check, then submit is rejected and writes no application/history | Transaction design retained; implementation test pending | `sbt 'IntegrationTest / test'` | Not implemented: requires replica-set transaction harness and `jobs.version` guard write |
| P2-AC07 | Given valid application status changes, when a recruiter changes status, then current status and immutable history are written atomically | `ApplicationServiceSpec`; `MongoHiringRepositoriesIntegrationSpec` | `sbt test`; `sbt 'IntegrationTest / test'` | PARTIAL: service and Mongo write/status history covered; optional session transaction path implemented, but replica-set atomicity/race proof not yet added |
| P2-AC08 | Given invalid transitions, missing rejection feedback, or missing decline reason, when status change is attempted, then no state/history changes are written | `ApplicationServiceSpec`; existing `ApplicationLifecycleSpec` | `sbt test` | PASS for service/domain no-write behavior with fake repositories; Mongo transaction no-write proof pending for failure injection |
| P2-AC09 | Given candidate and recruiter list requests, when optional status filters and `{createdAt,_id}` cursors are used, then results are authorization-scoped, bounded, deterministic, and backed by the named compound indexes with `explain(\"executionStats\")` evidence | Service list tests; Mongo index/list integration tests | `sbt test`; `sbt 'IntegrationTest / test'` | PARTIAL: scoped lists, bounds, named indexes, deterministic repository sort covered; `explain` executionStats evidence pending |
| P2-AC10 | Given a mid-transaction repository failure, when application submit or status change fails, then the database contains no partial write and the service returns a sanitized typed error | Planned fault-injection replica-set test | `sbt 'IntegrationTest / test'` | PARTIAL: optional session transaction wrapper added for application/event multi-writes; no replica-set fault-injection proof yet |
| P2-AC11 | Given Phase 2 completion, when local verification runs, then skills, unit tests, replica-set integration tests, whitespace check, and required independent reviews pass | Full local/review gate | `python3 scripts/check-skills.py`; `sbt test`; `sbt 'IntegrationTest / test'`; `git diff --check`; reviews | PARTIAL: skills, unit tests, standalone integration tests, and whitespace passed; replica-set transaction tests and independent reviews pending |
...
- **Scala Developer:** implement job `State` transitions, `ActorContext`, services, typed service errors, repository interfaces/adapters, and focused tests.
...
- Current status: partially implemented. Domain/application service slice and standalone Mongo repository/setup slice are complete with local evidence; transaction race/fault-injection criteria remain open.
- Completed baseline: Phase 2A domain foundation and evidence remain recorded above.
- Implementation checkpoint (2026-09-16): TDD red/green captured for `JobLifecycleSpec`; focused service red/green captured for missing `ActorContext`, union `UseCaseError`, services, and repository errors. Final local commands passed after review fixes: `sbt test 'IntegrationTest / test' && python3 scripts/check-skills.py && git diff --check` passed 113 unit tests, 30 integration tests, 8 skill validations, and whitespace check.
- Independent review checkpoint (2026-09-16): Code Reviewer FAIL, Security Engineer FAIL, and QA FAIL because replica-set transaction/race/fault-injection evidence, close-vs-submit guard-write implementation, and `explain("executionStats")` evidence remain open. Post-review fixes added non-seeded Admin marker rejection, stale Mongo replace conflict detection, current/target status error mapping, and optional session-backed transaction wrappers for application/event writes; these reduce defects but do not close Phase 2 completion.
- Next concrete action: add replica-set MongoDB transaction repository methods and synchronized/fault-injection tests for P2-AC05, P2-AC06, P2-AC07 atomicity, P2-AC09 explain evidence, and P2-AC10.
