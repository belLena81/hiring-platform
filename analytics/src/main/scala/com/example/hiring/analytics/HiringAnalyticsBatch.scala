...
object KafkaOffsetRangeSource {
  private[analytics] def offsetJson(ranges: Vector[PartitionOffsetRange], select: PartitionOffsetRange => Long): String = {
    val byTopic = ranges.groupBy(_.topic).toSeq.sortBy(_._1).map { case (topic, topicRanges) =>
      val partitions = topicRanges.sortBy(_.partition).map(range => s"\"${range.partition}\":${select(range)}").mkString(",")
      s"\"$topic\":{$partitions}"
    }.mkString(",")
    s"{$byTopic}"
  }
...
final case class AnalyticsFunnelDayOutput(day: Instant, created: Long, accepted: Long, declined: Long, interview: Long, hired: Long, rejected: Long)
final case class AnalyticsTimeToHireOutput(p50Hours: Double, p75Hours: Double, p90Hours: Double, p95Hours: Double, eligibleCount: Long, excludedCount: Long)
...
/**
  * Delta batch writer with idempotent natural keys. A completed manifest is written only after
  * Bronze, Silver, quarantine, and both rebuildable Gold datasets have been durably updated.
  */
final class HiringAnalyticsBatch(
  ...
  def run(spark: SparkSession, source: BoundedOperationalEventSource, manifest: AnalyticsRunManifest): AnalyticsPublication = {
    import spark.implicits._

    val rawRecords = source.read(spark, manifest)
    requireKafkaRecordColumns(rawRecords.schema)
    val startedAt = clock()
    val incomingBronze = withExpiry(OperationalEventTransforms.bronze(rawRecords), startedAt, AnalyticsRetention.BronzeDays)
    val bronzeRecords = incomingBronze.count()

    writeManifest(spark, manifest, "STARTED", startedAt.toString)
    merge(incomingBronze, paths.bronze, "target.topic = source.topic AND target.partition = source.partition AND target.offset = source.offset")
    ...
    val malformed = withExpiry(OperationalEventTransforms.malformedEvents(parsed)
      .withColumn("quarantineId", sha2(col("rawValue"), 256))
      .withColumn("quarantineReason", lit("INVALID_OPERATIONAL_EVENT_ENVELOPE")), startedAt, AnalyticsRetention.QuarantineDays)

    val validRecords = valid.count()
    val newConflicts = OperationalEventTransforms.conflictingEventIds(valid)
    val markerTokens = deletionMarkers.activeSubjectTokens(spark)
    ...
    val conflictQuarantine = withExpiry(valid.join(conflicts, Seq("eventId"), "inner")
      .withColumn("quarantineId", sha2(col("rawValue"), 256))
      .withColumn("quarantineReason", lit("CONFLICTING_EVENT_ID")), startedAt, AnalyticsRetention.QuarantineDays)
    ...
    val silver = withExpiry(incomingSilver.join(conflicts, Seq("eventId"), "left_anti"), startedAt, AnalyticsRetention.SilverDays)
    ...
    writeManifest(spark, manifest, "COMPLETED", completedAt.toString)
  ...
  private def writeManifest(spark: SparkSession, manifest: AnalyticsRunManifest, status: String, updatedAt: String): Unit = {
    import spark.implicits._
    val rows = manifest.offsetRanges.map(range => (manifest.runId, range.topic, range.partition, range.startOffset, range.endOffsetExclusive, status, updatedAt))
    val frame = rows.toDF("runId", "topic", "partition", "startOffset", "endOffsetExclusive", "status", "updatedAt")
    val condition = "target.runId = source.runId AND target.topic = source.topic AND target.partition = source.partition"
    if (DeltaTable.isDeltaTable(spark, paths.manifests))
      DeltaTable.forPath(spark, paths.manifests).as("target")
  ...
  private def merge(source: DataFrame, path: String, condition: String): Unit =
    if (DeltaTable.isDeltaTable(source.sparkSession, path))
      DeltaTable.forPath(source.sparkSession, path).as("target")
  ...
  private def withExpiry(frame: DataFrame, now: Instant, days: Int): DataFrame =
    frame.withColumn("ingestedAt", lit(Timestamp.from(now)))
  ...
  private def requireKafkaRecordColumns(schema: StructType): Unit = {
    ...
    require(missing.isEmpty, s"Kafka batch records are missing required columns: ${missing.toSeq.sorted.mkString(", ")}")
  }
}

/**
  * Bounded batch entry point:
  * `runId bootstrapServers lakehouseRoot topic partition startOffset endOffsetExclusive`.
...
object HiringAnalyticsBatchMain {
  def main(args: Array[String]): Unit = args.toList match {
    case runId :: bootstrapServers :: lakehouseRoot :: topic :: partition :: start :: end :: Nil =>
      val manifest = AnalyticsRunManifest(runId, Vector(PartitionOffsetRange(topic, partition.toInt, start.toLong, end.toLong)))
      val spark = SparkSession.builder()
        ...
        .getOrCreate()
      try {
      val hmacSecret = sys.env.getOrElse("HIRING_ANALYTICS_HMAC_SECRET_BASE64", sys.error("HIRING_ANALYTICS_HMAC_SECRET_BASE64 is required"))
      ...
      val emptyMarkers = new ActiveDeletionMarkerSource {
        override def activeSubjectTokens(spark: SparkSession): DataFrame = {
          ...
          spark.createDataFrame(spark.sparkContext.emptyRDD[org.apache.spark.sql.Row], StructType(Seq(StructField("subjectToken", StringType, nullable = false))))
        }
      }
    ...
    case _ =>
      sys.error("usage: HiringAnalyticsBatchMain runId bootstrapServers lakehouseRoot topic partition startOffset endOffsetExclusive")
  }
}
