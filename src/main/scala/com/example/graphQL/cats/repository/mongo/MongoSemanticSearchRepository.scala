...
final class MongoSemanticSearchRepository(
  ...
  override def candidateMatches(query: VectorSearchQuery): IO[Either[RepositoryError, List[RankedCandidate]]] = {
    ...
    val pipeline =
      List(vectorSearchStage(candidateVectorIndex, query.vector, filter, query.first.value), scoreStage).asJava
    PublisherBridge
      .collectWithin(users.aggregate(pipeline), query.first.value)
      .map { documents =>
...
private[mongo] object MongoSemanticSearchResult {
  ...
  def rankedCandidate(
      document: Document,
      query: VectorSearchQuery
  ): Either[NonEmptyList[MongoHiringCodecs.StoredDocumentError], Option[RankedCandidate]] =
    val stored = new Document(document)
    stored.remove("score")
