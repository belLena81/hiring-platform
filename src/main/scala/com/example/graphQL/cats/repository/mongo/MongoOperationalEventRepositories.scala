  ...
  private def recordInteractionWithSession(
    ...
    session
      .fold(
        ...
        PublisherBridge.first(outbox.insertOne(active, MongoHiringCodecs.outboxRecord(event, event.occurredAt)))
      )
      .as(Right(true))
      .handleErrorWith {
...
final class MongoOperationalEventOutboxRepository(database: MongoDatabase) extends OperationalEventOutboxRepository {
  private val outbox = database.getCollection("event_outbox")
  ...
  override def claim(
    ...
    List
      .fill(limit)(())
      .foldLeft(
        IO.pure(Right(List.empty[ClaimedOperationalEvent]): Either[RepositoryError, List[ClaimedOperationalEvent]])
      ) { case (acc, _) =>
        acc.flatMap {
