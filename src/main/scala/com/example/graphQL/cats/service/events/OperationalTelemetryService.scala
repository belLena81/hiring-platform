...
import com.example.graphQL.cats.service.protocol.{IdempotencyRequest, InteractionUseCases, UseCaseIO, UseCaseIO as UseCase}
...
final class OperationalTelemetryService(
  ...
  override def recordJobView(
    ...
    idempotent.execute[Unit]("recordJobView", Idempotent.actorScope(actor), request, _ => interactionReference(eventId), replayInteraction(actor, eventId)) { context =>
      for {
        ...
        rank <- searchId.fold(UseCase.pure(Option.empty[Int]))(id => verifiedSearchResult(actor, id, jobId.value.toString).map(Some(_)))
  ...
  override def recordSearchResultClick(
    ...
    idempotent.execute[Unit]("recordSearchResultClick", Idempotent.actorScope(actor), request, _ => interactionReference(eventId), replayInteraction(actor, eventId)) { context =>
      for {
  ...
  private def verifiedSearchResult(
    ...
    UseCase.fromIO(searchSessions.find(searchId).flatMap(_.widenUseCase.fold(
  ...
  private def interactionReference(eventId: UUID): com.example.graphQL.cats.repository.protocol.MutationEntityReference =
