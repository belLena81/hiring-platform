...
class ApplicationServiceSpec extends CatsEffectSuite {
  ...
  test("submitApplication derives candidate from ActorContext and writes initial history") {
    withServices(Map(jobId -> openJob), Map.empty, now, List(applicationId.value, eventId.value)).flatMap { case (_, applications, service) =>
      for {
        result <- service.submitApplication(request("submit"), ActorContext(candidateId, UserRole.Candidate), jobId).value
  ...
  test("closed jobs reject submission without writing application history") {
    withServices(Map(jobId -> openJob.copy(status = JobStatus.Closed)), Map.empty, now, List(applicationId.value, eventId.value)).flatMap {
      case (_, applications, service) =>
      for {
        result <- service.submitApplication(request("submit-closed"), ActorContext(candidateId, UserRole.Candidate), jobId).value
        events <- applications.allEvents
      } yield {
  ...
  test("submitApplication preserves repository conflict when job closes before transactional write") {
    withServices(Map(jobId -> openJob), Map.empty, now, List(applicationId.value, eventId.value)).flatMap { case (_, applications, service) =>
      for {
        ...
        result <- service.submitApplication(request("submit-conflict"), ActorContext(candidateId, UserRole.Candidate), jobId).value
        events <- applications.allEvents
      } yield {
  ...
  test("owned recruiter status change updates application and appends immutable history") {
    withServices(Map(jobId -> openJob), Map(applicationId -> createdApplication), later, List(eventId.value)).flatMap { case (_, applications, service) =>
      for {
  ...
  test("hireApplication emits status-change before candidate-hired with distinct event IDs") {
    ...
    withServices(Map(jobId -> openJob), Map(applicationId -> interviewApplication), later, List(eventId.value)).flatMap { case (_, applications, service) =>
      for {
        result <- service.changeStatus(
          request("hire"),
          ActorContext(recruiterId, UserRole.Recruiter),
  ...
  test("outbox failure rolls back application submission at the repository boundary") {
    withServices(Map(jobId -> openJob), Map.empty, now, List(applicationId.value, eventId.value)).flatMap { case (_, applications, service) =>
      for {
        ...
        result <- service.submitApplication(request("submit-outbox-failure"), ActorContext(candidateId, UserRole.Candidate), jobId).value
        events <- applications.allEvents
        outbox <- applications.allOperationalEvents
      } yield {
  ...
  test("myApplications scopes list access to the actor candidate") {
    withServices(Map(jobId -> openJob), Map(applicationId -> createdApplication), now, Nil).flatMap { case (_, _, service) =>
