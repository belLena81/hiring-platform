...
private[cats] object ServiceFixtures {
        ...
        .map(
          _.filter(event =>
            event.applicationId == applicationId &&
              keysetAfter(page.cursor.map(cursor => cursor.occurredAt -> cursor.id.value.toString))(event)(
                _.occurredAt,
                _.id.value.toString
              )
          ).toList
  ...
  private def keysetAfter[A](
      cursor: Option[(Instant, String)]
  )(value: A)(timestamp: A => Instant, id: A => String): Boolean =
