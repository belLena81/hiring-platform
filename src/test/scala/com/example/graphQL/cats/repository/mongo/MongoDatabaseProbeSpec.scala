  ...
  test("HTTP readiness through the real Mongo adapter preserves correlation across equal timeout budgets") {
    cats.effect.Resource.fromAutoCloseable(IO.blocking(new ServerSocket(0))).use { socket =>
      for {
        ...
        _ <- MongoDatabaseProbe.resource(s"mongodb://127.0.0.1:${socket.getLocalPort}", "foundation", sink).use {
          probe =>
            for {
              dependencies <- TestGraphQLSupport.dependencies().allocated.map(_._1)
              http <- new HiringApiRoutes(new HealthService(probe, sink), sink, dependencies)
    ...
    cats.effect.Resource.fromAutoCloseable(IO.blocking(new ServerSocket(0))).use { socket =>
      for {
        ...
        sink = new Diagnostics {
          def event(event: LogEvent, id: Option[String], fields: => Map[LogField, String]): IO[Unit] =
            events
              .update(_ :+ ((event, id, fields))) *> IO.raiseError(new IllegalStateException("synthetic-sink-secret"))
        }
        id = Some("fe211944-7015-4e73-8dc1-000000000001")
        result <- MongoDatabaseProbe
          .resource(
