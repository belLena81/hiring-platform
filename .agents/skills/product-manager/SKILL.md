...
1. Inspect implementation and relevant use cases. Distinguish implemented behavior from target design. Select one vertical slice and explain its user value, phase, dependencies, non-goals, and observable acceptance criteria.
   Require business-meaningful names in specs, acceptance criteria, handoffs, and proposed code. Avoid names based on project phases, scaffolding, temporary architecture layers, or the development process when a domain capability name is available.
