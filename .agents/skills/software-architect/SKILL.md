...
- Keep OLTP independent of Kafka, Spark, Databricks, and embedding-provider availability. Where Kafka publication is required, the architecture must use an event-driven architecture pattern with a durable handoff, such as a transactional outbox, versioned event envelopes, idempotent delivery, and explicit recovery.
